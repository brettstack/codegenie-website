...
<figure style={{margin: '2rem auto', width: '360px'}}>
  <Image
    ...
    loading='eager'
  />
  <figcaption style={{margin: 0, color: '#666', fontSize: '0.8rem'}}>Pretty picture of the Login UI we build in the [React Cognito User Pools](/blogaws-cognito-user-pools-add-auth-to-a-react-app) article.</figcaption>
</figure>
...
import { getEnvironmentConfig, getIsDeletionProtectionEnabled, getIsSourceMapsEnabled, getRemovalPolicy } from '../environment-config'
...
export default class Auth extends Construct {
  ...
  // See the other tabs above for the remaining code
}
```
...
:::note
[Scroll up](#cognito-user-pool-cdk-iac) to see the other code tabs
:::

</TabItem>
<TabItem label='User Pool Client'>
```typescript {15-19}
...
<TabItem label='Identity Providers'>
```typescript {9-17}
...
<TabItem label='Environment Config'>
```typescript
