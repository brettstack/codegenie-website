...
Building a full stack Serverless app on AWS takes a lot of time, effort, and expertise. Code Genie lets you get up and running fast with a solid software foundation based on your data model. In mere minutes you can have a full stack application (like the one described above) deployed to your own AWS account (and the source code downloaded for you to start hacking). Check out the [Getting Started Guide](/docs/guides/getting-started) for more details.

*Coming Soon*: We're currently working on adding support for dashboards and alarming similar to what's described in this post so that Code Genie Apps will have this functionality out-of-the-box.