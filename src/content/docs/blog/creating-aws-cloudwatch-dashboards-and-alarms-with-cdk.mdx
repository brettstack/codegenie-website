...
<figure>
  <Image
    ...
    loading='eager'
  />
  <figcaption style={{margin: 0, color: '#666', fontSize: '0.8rem'}}>Example dashboard that would look so much more interesting if it had actual data.</figcaption>
</figure>

## Monitoring Construct

Enough preamble. Let's get into the code.
...
export default class Monitoring extends Construct {
  ...
  constructor(scope: Construct, id: string, props: DashboardProps) {
    ...
    this.alarmSnsTopic.addSubscription(
      new EmailSubscription(ALARM_NOTIFICATION_EMAIL),
    )
...
The Monitoring class receives details on other resources within our app such as our Amplify Hosting App, API Gateway, Cognito User Pool and Client, and a list of Lambda Functions and DynamoDB Tables we want to monitor.
...
We're also creating the `Dashboard` resource and calling methods to add widgets and alarms (we'll get to the implementation of these next).
...
The main difference with the Lambda metrics is that we're dealing with **multiple** Lambda Functions (as opposed to a single API Gateway Resource). Each graph includes the metrics for all of the Lambda Functions passed into the Monitoring construct. Alternatively, you could use `functions.map` to instead create one graph per function.
...
We're actually creating 2 alarms per Lambda Function for throttle and error.  These alarms aren't set up to notify; for that we creat a composite alarm so that we don't get inundated with notifications when a single issue causes multiple functions to fail.
