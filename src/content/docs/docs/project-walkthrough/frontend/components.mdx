...
- `foreignKey` properties are rendered as a searchable [Select](https://ant.design/components/select) with a list of records of the defined Entity (`foreignKey.referencedEntity`). If the Foreign Entity defines an `imageProperty`, the select options are rendered as an [`<AvatarNameLink>`](#avatarnamelink). As you type, queries are made to the [Search API]() via the [useSearch*Query hook](./hooks#usesearch-query) and are debounced and cached.
